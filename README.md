...
_`practices/*.md` と `antipatterns/*.md` から `pnpm gen` で生成。このブロックは編集しないこと。_ **10** 件の active な practice と **0** 件の active な antipattern を [`practices/index.md`](practices/index.md) / [`antipatterns/index.md`](antipatterns/index.md) に索引化(ライセンス: [CC BY 4.0](LICENSE-DOCS))。
...
_Generated from `practices/*.md` and `antipatterns/*.md` by `pnpm gen`; do not edit this block._ **10** active practices and **0** active antipatterns are indexed in [`practices/index.md`](practices/index.md) and [`antipatterns/index.md`](antipatterns/index.md), licensed under [CC BY 4.0](LICENSE-DOCS).
